...
**当前阶段**：Phase G/H/I/J 完结。按 `docs/PROJECT_REVIEW_2026_06_10.md` 四阶段路线图推进，**目标定位＝简历/答辩展示**。**阶段一 安全止血**（§5.1 T1/T2/T4 ☑、T3 部分）+ **阶段二 瘦身**（§5.2 T5/T7/T8/T9 ☑、T6/T10 不采纳）均已收尾（2026-06-19，详见顶部"最近更新"）。亮点链已定型＝**Agent 风险画像全链**（AgentLoop 默认主路径 + 2 MCP + RAG 真灌库）。
**下一步**：两条线，按优先级——
- **① 用户侧运行验证（亮点链落地）**：起栈（`docs/educare/E2E_RUNBOOK.md`）+ 本地 llama.cpp + Milvus 灌库（`/api/v1/rag/upsert`，`RAG_UPSERT_DESIGN.md`），跑 `scripts/mcp_smoke_test.sh` + 一次真实 `/agent/api/v1/task/trigger/{id}`，确认「风险画像→ReAct→MCP 取数+RAG→干预方案」端到端 + Langfuse 出 trace；同时验证 MCP token 链（设 `EDUCARE_MCP_TOKEN` 三进程）+ 补 8095(FastMCP) 服务端 token 校验。
- **② Phase 3 规范健壮性（review §5.3，可并行）**：T12 `GlobalExceptionHandler` 下沉 common（A7）、T13 Python CORS 收白名单/关 credentials（A4）、T14 密钥无默认 env+fail-fast（A5）、T16 `educare.field-permission.enabled` 默认开（也是安全亮点）；以及剩余安全收口 agent-service:8087 自身鉴权深防御。**Phase 4 工程卫生**（A9 .pyc 清理 / A10 PII 目录 / A12 / A13）随后。
...
- ~~**遗留待办**：`/agent/api/v1/_internal/loop/dry-run` 零 auth~~ **已闭（2026-06-15）**：网关 `JwtAuthGlobalFilter` 对所有 `/_internal/` 路径一律 403，公网网关不再暴露内网手测端点
