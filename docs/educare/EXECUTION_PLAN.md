...
**当前阶段**：Phase G/H/I/J 规划项全部完结（Phase J 完成于 2026-06-06，见 §10）。按 `docs/PROJECT_REVIEW_2026_06_10.md` 四阶段路线图推进，**目标定位＝简历/答辩展示**（A 类安全必做；B 类挑 1-2 个端到端亮点，其余默认关半成品删）。**阶段一 安全止血已收尾**（T1/T2/T4 ☑、T3 部分；2026-06-19，详见顶部"最近更新"）
**下一步**：进入 **Phase 2 阶段二 瘦身**（review §5.2）—— **保留并端到端跑通作亮点**：AgentLoop 设默认路径 + 2 个 MCP server（8094/8095）+ RAG 真灌库（一条「风险画像→多轮 ReAct→MCP 取数+RAG→干预方案」演示链），并对 8094/8095 加内部预共享 token（消解 A3 余项 / T6）；**删除/降级半成品**：T5 四层记忆+memory MCP(8096)、T7 ModelRouter+cloudChatClient、T8 灰度百分比降单布尔、T9 Hybrid Retrieval+Skills 热更新简化。**先与用户确认保留哪条亮点链** 再动手删除。**剩余安全收口**（可并入阶段三）：agent-service:8087 自身鉴权深防御、`educare.field-permission.enabled` 默认开（A8/T16）
