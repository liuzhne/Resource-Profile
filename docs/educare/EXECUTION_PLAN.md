...
**当前阶段**：Phase H（MCP 化 + Agent Loop 重构）—— H-1 / H-2 / H-3 / H-4 全部完结
**下一步**：→ §3 H-5 Mem0 + memory-server（视进度项）。H-5.1 选型决策（Mem0 vs Letta 稳定性）→ H-5.2 记忆分层（Working/Episodic/Semantic/Procedural）→ H-5.3 memory-server MCP（Python，端口建议 8096）暴露 `recall_student_history / save_episode / summarize_long_term`，与 knowledge-rag（8095）对称。建议复用 ai-inference-service 的 redis/milvus client，记忆持久化先走 Redis（Working/Episodic）+ Milvus（Semantic）
